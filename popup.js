document.addEventListener('DOMContentLoaded', () => {
    ...
    // Load from Storage
    chrome.storage.sync.get(['cs_panel_settings', 'cs_comment_sort_mode', 'cs_auto_next', 'cs_watch_later'], (data) => {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs[0]) {
                chrome.tabs.sendMessage(tabs[0].id, {
                    ...
                    settings: { panelSettings, commentSortMode, autoNextEnabled }
                }).catch(() => {});
            }
        });
    }
    ...
    document.getElementById('toggle-topbar').addEventListener('click', () => { panelSettings.showTopBar = !panelSettings.showTopBar; syncUI(); saveSettings(); });
    document.getElementById('toggle-stories').addEventListener('click', () => { panelSettings.showStories = !panelSettings.showStories; syncUI(); saveSettings(); });
    document.getElementById('toggle-leftrail').addEventListener('click', () => { panelSettings.showLeftRail = !panelSettings.showLeftRail; syncUI(); saveSettings(); });
    document.getElementById('toggle-rightrail').addEventListener('click', () => { panelSettings.showRightRail = !panelSettings.showRightRail; syncUI(); saveSettings(); });
    document.getElementById('toggle-comments').addEventListener('click', () => { panelSettings.showComments = !panelSettings.showComments; syncUI(); saveSettings(); });
    ...
    document.getElementById('speed-select').addEventListener('change', (e) => {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs[0]) chrome.tabs.sendMessage(tabs[0].id, { action: 'SET_SPEED', speed }).catch(() => {});
        });
    });
    ...
    function renderWatchLater() {
        ...
        if (watchLaterList.length === 0) {
            listEl.innerHTML = '<div style="color:#888; text-align:center; padding:15px; font-size:11px;">Queue is empty.<br>Click Save Current Video to add!</div>';
            return;
        }
        ...
        watchLaterList.forEach(item => {
            ...
            el.innerHTML = `
                ...
                <div style="font-size:9px; color:#888; display:flex; justify-content:space-between;">
                    <span>${item.date}</span>
                    <span style="color:var(--accent-cyan);">${item.site}</span>
                </div>
    ...
    document.getElementById('save-current-btn').addEventListener('click', () => {
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            ...
            chrome.tabs.sendMessage(tabs[0].id, { action: 'GET_METADATA' }, (res) => {
                ...
                if (exists) return alert('Video already saved in Watch Later!');
